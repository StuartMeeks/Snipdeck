...
namespace Snipdeck.Execution.Tests.Services
{
    public sealed class SqliteCommandHistoryStoreTests : IDisposable
    {
        ...
        public async Task Added_run_round_trips_through_query_and_get()
        {
            ...
            await _store.AddAsync(entry, retentionPerSnip: 50);

            var all = await _store.QueryAsync();
            ...
            var fetched = await _store.GetAsync(entry.Id);
        ...
        {
            var snip = Guid.NewGuid();
            await _store.AddAsync(Entry(snip, command: "old", minutesAfterBase: 0), 50);
            await _store.AddAsync(Entry(snip, command: "middle", minutesAfterBase: 5), 50);
            await _store.AddAsync(Entry(snip, command: "newest", minutesAfterBase: 10), 50);
        ...
        public async Task Query_filters_by_snip()
        {
            ...
            await _store.AddAsync(Entry(snipA, command: "a-cmd"), 50);
            await _store.AddAsync(Entry(snipB, command: "b-cmd"), 50);

            var onlyA = await _store.QueryAsync(snipId: snipA);
        ...
        {
            var snip = Guid.NewGuid();
            await _store.AddAsync(Entry(snip, command: "kubectl describe pod", output: "Running", minutesAfterBase: 0), 50);
            await _store.AddAsync(Entry(snip, command: "git status", output: "clean tree", minutesAfterBase: 1), 50);

            var byCommand = await _store.QueryAsync(search: "KUBECTL");
            Assert.Equal("kubectl describe pod", Assert.Single(byCommand).ResolvedCommand);

            var byOutput = await _store.QueryAsync(search: "CLEAN");
        ...
        public async Task Add_prunes_oldest_runs_beyond_retention_per_snip()
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                await _store.AddAsync(Entry(snip, command: $"run-{i}", minutesAfterBase: i), retentionPerSnip: 3);
            }

            var all = await _store.QueryAsync(snipId: snip);
        ...
        public async Task Retention_is_scoped_per_snip()
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                await _store.AddAsync(Entry(snipA, command: $"a-{i}", minutesAfterBase: i), retentionPerSnip: 2);
                await _store.AddAsync(Entry(snipB, command: $"b-{i}", minutesAfterBase: i), retentionPerSnip: 2);
            }

            Assert.Equal(2, (await _store.QueryAsync(snipId: snipA)).Count);
            Assert.Equal(2, (await _store.QueryAsync(snipId: snipB)).Count);
        }
        ...
        public async Task Retention_of_zero_or_less_skips_pruning()
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                await _store.AddAsync(Entry(snip, command: $"run-{i}", minutesAfterBase: i), retentionPerSnip: 0);
            }

            Assert.Equal(4, (await _store.QueryAsync(snipId: snip)).Count);
        }
        ...
        public async Task Delete_removes_a_single_run()
        {
            ...
            await _store.AddAsync(keep, 50);
            await _store.AddAsync(drop, 50);

            await _store.DeleteAsync(drop.Id);
        ...
        {
            var snip = Guid.NewGuid();
            await _store.AddAsync(Entry(snip), 50);
            await _store.AddAsync(Entry(snip, minutesAfterBase: 1), 50);

            await _store.ClearAsync();

            Assert.Empty(await _store.QueryAsync());
        }
        ...
        public async Task Null_raw_stream_round_trips_as_null()
        {
            ...
            await _store.AddAsync(entry, 50);

            var stored = await _store.GetAsync(entry.Id);
